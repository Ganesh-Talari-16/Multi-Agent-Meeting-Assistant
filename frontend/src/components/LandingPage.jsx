...
export default function LandingPage({ onOpenAuth, onLaunchApp, isUserLoggedIn }) {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans selection:bg-sky-500 selection:text-white relative overflow-hidden">
      ...
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full max-w-7xl h-[600px] bg-gradient-to-b from-sky-600/20 via-indigo-600/10 to-transparent blur-3xl pointer-events-none -z-10" />
      ...
      <header className="sticky top-0 z-40 bg-slate-950/80 backdrop-blur-xl border-b border-slate-800/80">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 cursor-pointer" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
            ...
            <div>
              <span className="text-base font-extrabold tracking-tight bg-gradient-to-r from-white via-slate-200 to-sky-300 bg-clip-text text-transparent">
                MeetingAI Assistant
              </span>
          ...
          <div className="flex items-center gap-3">
            {isUserLoggedIn ? (
              <button
                onClick={onLaunchApp}
                className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-sky-500 to-blue-600 text-white text-xs font-bold shadow-lg shadow-sky-500/20 hover:shadow-sky-500/35 hover:-translate-y-0.5 transition-all flex items-center gap-2"
              >
              ...
              <>
                <button
                  ...
                  className="px-4 py-2 rounded-xl text-xs font-bold text-slate-300 hover:text-white hover:bg-slate-900 border border-transparent hover:border-slate-800 transition-all flex items-center gap-2"
                >
      ...
      <section className="pt-20 pb-24 px-6 max-w-7xl mx-auto text-center relative">
        {/* Release Pill */}
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-sky-950/60 border border-sky-500/30 text-sky-300 text-xs font-semibold mb-8 backdrop-blur-md">
        ...
        <div className="flex flex-wrap items-center justify-center gap-4 mb-16">
          ...
          <button
            ...
            className="px-7 py-4 rounded-2xl bg-slate-900/90 text-slate-200 border border-slate-800 text-sm font-bold hover:bg-slate-850 hover:border-slate-700 transition-all flex items-center gap-2.5"
          >
        ...
        {/* Trust Badges Bar */}
        <div className="pt-8 border-t border-slate-800/60 max-w-4xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-6 text-slate-400 text-xs font-medium">
            ...
            {activeDemoTab === 'summarizer' && (
              <div className="space-y-4">
                <div className="flex items-center justify-between p-3 rounded-xl bg-slate-950 border border-slate-800 text-xs">
                  ...
                  <span className="text-slate-400 font-mono text-[11px]">Duration: 42m 15s</span>
                </div>
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  <div className="p-4 rounded-xl bg-slate-950/40 border border-slate-800 text-xs">
                    <h5 className="font-bold text-slate-200 mb-2 flex items-center gap-2">
            ...
            {activeDemoTab === 'action-items' && (
              <div className="space-y-3">
                ...
                {[
                  ...
                  <div key={idx} className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 flex flex-wrap items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                      <CheckCircle2 className={`w-5 h-5 ${item.status === 'Completed' ? 'text-emerald-400' : item.status === 'In Progress' ? 'text-amber-400' : 'text-slate-500'}`} />
                      <div>
                        <div className="text-xs font-bold text-slate-200">{item.title}</div>
                        <div className="text-[11px] text-slate-400 flex items-center gap-2 mt-0.5">
                          <span>Assignee: <strong className="text-sky-300">{item.assignee}</strong></span>
                          <span>•</span>
                    ...
                    <div className="flex items-center gap-2">
                      <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold border ${item.priority === 'High' ? 'bg-rose-500/10 text-rose-400 border-rose-500/20' : 'bg-amber-500/10 text-amber-400 border-amber-500/20'}`}>
              ...
              <div className="space-y-4">
                <form onSubmit={handleDemoQuerySubmit} className="relative">
                  <input
                    ...
                    className="w-full bg-slate-950 border border-slate-700 rounded-xl pl-4 pr-32 py-3 text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:border-sky-500"
                  />
                  <button
                    type="submit"
                    disabled={isQuerying}
                ...
                {demoQueryResult && (
                  <div className="p-5 rounded-2xl bg-slate-950/80 border border-sky-500/30 space-y-4">
                    ...
                    <div className="border-t border-slate-800 pt-3 space-y-2">
                      <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider block">Verbatim Citations (ChromaDB Vector Match):</span>
                      {demoQueryResult.citations.map((c, i) => (
                        <div key={i} className="p-3 rounded-lg bg-slate-900 border border-slate-800 text-[11px]">
                          <div className="text-sky-300 font-semibold mb-1">📄 {c.source}</div>
                          <div className="text-slate-400 italic font-mono">"{c.snippet}"</div>
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            {activeDemoTab === 'analytics' && (
              <div className="grid sm:grid-cols-3 gap-4">
                <div className="p-5 rounded-2xl bg-slate-950 border border-slate-800 text-center space-y-1">
                  <div className="text-3xl font-extrabold text-sky-400">98.4%</div>
                  <div className="text-xs font-bold text-slate-200">Action Item Completion</div>
                  <div className="text-[11px] text-slate-400">Measured across 1,200+ team syncs</div>
                </div>
                <div className="p-5 rounded-2xl bg-slate-950 border border-slate-800 text-center space-y-1">
                  <div className="text-3xl font-extrabold text-emerald-400">4.5 hrs</div>
                  <div className="text-xs font-bold text-slate-200">Saved Per Engineer / Wk</div>
                  <div className="text-[11px] text-slate-400">Eliminating manual meeting notes</div>
                </div>
                <div className="p-5 rounded-2xl bg-slate-950 border border-slate-800 text-center space-y-1">
                  <div className="text-3xl font-extrabold text-indigo-400">&lt; 350ms</div>
                  <div className="text-xs font-bold text-slate-200">Vector Search Latency</div>
                  <div className="text-[11px] text-slate-400">Hybrid ChromaDB RAG index</div>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {features.map((feat, idx) => {
            ...
            return (
              <div
                key={idx}
                className="p-7 rounded-3xl bg-slate-900/60 border border-slate-800/80 hover:border-sky-500/40 hover:-translate-y-1 transition-all group backdrop-blur-md"
              >
                ...
                <p className="text-xs text-slate-400 leading-relaxed font-normal">{feat.description}</p>
              </div>
            );
          })}
        </div>
      ...
      <section id="workflow" className="py-20 px-6 max-w-7xl mx-auto border-t border-slate-800/60">
        <div className="text-center mb-16">
          <h2 className="text-2xl sm:text-4xl font-bold tracking-tight mb-4">
        ...
        <div className="grid md:grid-cols-3 gap-8 relative">
          <div className="p-8 rounded-3xl bg-slate-900/40 border border-slate-800 space-y-4">
            <div className="w-10 h-10 rounded-full bg-sky-500/10 border border-sky-500/30 text-sky-400 font-extrabold text-sm flex items-center justify-center">
              01
            </div>
            <h4 className="text-base font-bold text-slate-100">Upload or Record Audio</h4>
          ...
          <div className="p-8 rounded-3xl bg-slate-900/40 border border-slate-800 space-y-4">
            <div className="w-10 h-10 rounded-full bg-indigo-500/10 border border-indigo-500/30 text-indigo-400 font-extrabold text-sm flex items-center justify-center">
              02
            </div>
            <h4 className="text-base font-bold text-slate-100">Multi-Agents Extract Insights</h4>
            <p className="text-xs text-slate-400 leading-relaxed">
          ...
          <div className="p-8 rounded-3xl bg-slate-900/40 border border-slate-800 space-y-4">
            ...
            <h4 className="text-base font-bold text-slate-100">Query RAG & Track Action Items</h4>
            <p className="text-xs text-slate-400 leading-relaxed">
      ...
      <section id="pricing" className="py-20 px-6 max-w-7xl mx-auto border-t border-slate-800/60">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          ...
          <div className="p-8 rounded-3xl bg-slate-900/50 border border-slate-800 flex flex-col justify-between space-y-6">
            <div>
              <div className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Starter / Trial</div>
              <div className="text-3xl font-extrabold text-slate-100 mb-1">$0 <span className="text-xs text-slate-400 font-normal">/ month</span></div>
              <p className="text-xs text-slate-400 mb-6">Perfect for small teams and quick evaluation.</p>
              <ul className="space-y-3 text-xs text-slate-300">
                <li className="flex items-center gap-2.5"><Check className="w-4 h-4 text-emerald-400" /> Up to 10 meeting recordings/mo</li>
                <li className="flex items-center gap-2.5"><Check className="w-4 h-4 text-emerald-400" /> Multi-Agent Summaries</li>
                <li className="flex items-center gap-2.5"><Check className="w-4 h-4 text-emerald-400" /> Action Item Extraction</li>
                <li className="flex items-center gap-2.5"><Check className="w-4 h-4 text-emerald-400" /> Basic RAG Search</li>
              </ul>
            </div>
            <button
              onClick={() => onOpenAuth('register')}
              className="w-full py-3 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-100 text-xs font-bold transition-colors"
            >
              Start Free Trial
            </button>
          </div>
          ...
          <div className="p-8 rounded-3xl bg-gradient-to-b from-sky-950/80 via-slate-900 to-slate-900 border-2 border-sky-500 shadow-xl shadow-sky-500/10 flex flex-col justify-between space-y-6 relative">
            ...
            <div>
              <div className="text-xs font-bold text-sky-400 uppercase tracking-wider mb-2">Pro Team</div>
              <div className="text-3xl font-extrabold text-slate-100 mb-1">$29 <span className="text-xs text-slate-400 font-normal">/ user / mo</span></div>
              <p className="text-xs text-slate-300 mb-6">For growing product & engineering teams.</p>
              <ul className="space-y-3 text-xs text-slate-200">
                <li className="flex items-center gap-2.5"><Check className="w-4 h-4 text-sky-400" /> Unlimited meeting recordings</li>
                <li className="flex items-center gap-2.5"><Check className="w-4 h-4 text-sky-400" /> Full ChromaDB Vector RAG Search</li>
                <li className="flex items-center gap-2.5"><Check className="w-4 h-4 text-sky-400" /> Advanced Speaker Diarization</li>
                <li className="flex items-center gap-2.5"><Check className="w-4 h-4 text-sky-400" /> Automated Deadline Warnings</li>
                <li className="flex items-center gap-2.5"><Check className="w-4 h-4 text-sky-400" /> PDF Report Exports</li>
              </ul>
            </div>
            <button
              onClick={() => onOpenAuth('register')}
          ...
          <div className="p-8 rounded-3xl bg-slate-900/50 border border-slate-800 flex flex-col justify-between space-y-6">
            <div>
              ...
              <div className="text-3xl font-extrabold text-slate-100 mb-1">Custom</div>
              <p className="text-xs text-slate-400 mb-6">For organizations requiring strict security & custom LLM deployment.</p>
              <ul className="space-y-3 text-xs text-slate-300">
                <li className="flex items-center gap-2.5"><Check className="w-4 h-4 text-emerald-400" /> Custom On-Prem / VPC Deployment</li>
                <li className="flex items-center gap-2.5"><Check className="w-4 h-4 text-emerald-400" /> Dedicated RBAC Roles & SSO</li>
                <li className="flex items-center gap-2.5"><Check className="w-4 h-4 text-emerald-400" /> Zero Data Retention Guarantee</li>
                <li className="flex items-center gap-2.5"><Check className="w-4 h-4 text-emerald-400" /> 24/7 Dedicated Support SLA</li>
              </ul>
            </div>
            <button
              onClick={() => onOpenAuth('register')}
      ...
      {/* FAQ Section */}
      <section id="faq" className="py-20 px-6 max-w-4xl mx-auto border-t border-slate-800/60">
        <div className="text-center mb-12">
          <h2 className="text-2xl sm:text-4xl font-bold tracking-tight mb-4">
          ...
          {faqs.map((faq, idx) => (
            ...
            >
              <button
                ...
                className="w-full p-5 text-left flex items-center justify-between text-xs sm:text-sm font-bold text-slate-100"
              >
                <div className="flex items-center gap-3">
      ...
      {/* Footer */}
      <footer className="border-t border-slate-800/80 py-12 px-6 text-slate-500 text-xs">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-3">
            ...
            <span className="font-bold text-slate-300">Multi-Agent Meeting Assistant</span>
          </div>
